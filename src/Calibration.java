...
public class Calibration extends OpenCVTestBase
{
	...
	public Calibration()
	{
		...
		addCheckBox("Find chessboard:").addActionListener(e ->
		{
			_isScreenShotClick = false;
			_isFindChessBoard = ((JCheckBox)e.getSource()).isSelected();
		});
		
		addButton("Screenshot").addActionListener(e -> _isScreenShotClick = true);
		
		addButton("Delete all").addActionListener(e -> removeAllScreenshots());
		
		addButton("Calib left").addActionListener(e -> calibrateSingle(_camera1, _leftCameraImagesPathTF.getText(), _windowCalibrate1, ConfigLoader.LEFT_CAMERA_CALIBRATION_FILE));
		
		addButton("Calib right").addActionListener(e -> calibrateSingle(_camera2, _rightCameraImagesPathTF.getText(), _windowCalibrate2, ConfigLoader.RIGHT_CAMERA_CALIBRATION_FILE));
		
		addButton("Calib stereo").addActionListener(e ->
		{
			if(_camera1.isCalibrated() && _camera2.isCalibrated())
			{
		...
		addButton("Load single").addActionListener(e ->
		{
		...
		addButton("Load stereo").addActionListener(e -> System.out.println("Load stereo calibration configuration"));
